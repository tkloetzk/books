...
const styles = {
  ...
  improperData: {
    cursor: 'pointer',
    boxShadow: '0px 0px 3px 6px red, 0px 1px 1px 2px red, 0px 2px 1px 1px red',
  },
  owned: {
...
export class Book extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    const { book, originalBook, edits, saveIcon } = this.state;
    ...
    if (
      prevState.book.categories.includes('undefined') &&
      !book.categories.includes('undefined') &&
      !isEmpty(book.categories)
    ) {
      this.setState({ improperData: false });
      handleSave(book, [{ key: 'categories', newValue: book.categories }]);
    }
  ...
  updateOriginalBook = () => {
    ...
    let improperData = false;

    const mergedBook = Object.assign({}, this.state.book, book);

    if (isEmpty(mergedBook.categories)) {
      mergedBook.categories = ['undefined'];
      console.log('set to true');
      improperData = true;
    }
    this.setState({ book: mergedBook, originalBook: mergedBook, improperData });
  };
  ...
  render() {
    ...
    const { book, expanded, saveIcon, edits, improperData } = this.state;
    ...
    console.log('improperData', improperData);
    return (
      <Card
        className={[
          book.differences.length ? classes.different : null,
          improperData ? classes.improperData : null,
...
export default connect(mapStateToProps)(withStyles(styles)(Book2));
